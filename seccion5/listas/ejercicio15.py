...
#
# ¿Qué modificación habría que hacer en las tablas
# para guardar todos los resultados de todas las jornadas de la temporada?

num_equipos = 15
equipos = []
resultados = []
# Recorro las listas para incializar el nombre de los dos quipos y el resultado del partido
for indice in range(0, num_equipos):
    ...
    goles = []
    goles.append(int(input("Introduce los goles metidos por el equipo {0}:".format(partido[0]))))
    goles.append(int(input("Introduce los goles metidos por el equipo {0}:".format(partido[1]))))
    resultados.append(goles)

print("QUINIELA")
print("--------")

# Recorro las listas, muestro el nombre de los equipos
